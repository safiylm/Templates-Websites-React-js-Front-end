import '../../../styles/template9.scss'
function TemplateWebSite9() {
    return (
        <div className="page-template9">
            <div className='div0'>
                <img src="https://images01.nicepagecdn.com/c461c07a441a5d220e8feb1a/9ccfd8184e705bad974787e7/ghgh-min.jpg" />
                <div>
                    <h1> Delicious Breads</h1>
                    <p>Nibh venenatis cras sed felis eget velit aliquet sagittis id. <br /> Tellus pellentesque eu tincidunt tortor aliquam nulla.</p>
                    <p>Image from Freepik </p>
                    <button>VIEW PRODUCTS </button>
                </div>
            </div>

            <div className='div1'>
                <h1>Our Bread</h1>
                <p>
                    Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. <br />
                    Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt.</p>
                <div className='div-contanier-img-texte'>
                    <div className="div-img-texte">
                        <img src="https://images01.nicepagecdn.com/c461c07a441a5d220e8feb1a/114eb3a7d170559dbf141d3b/r.jpg" />
                        <div className="div-texte">
                            <h3 className="titre"> baguette</h3>
                            <p className="texte">Small-batch sourdough baguette.</p>
                            <div className="prix">$4.5</div>
                        </div>
                    </div>

                    <div className="div-img-texte">
                        <img src="https://images01.nicepagecdn.com/c461c07a441a5d220e8feb1a/0cd587018da1567ca82ec0a8/rr.jpg" />
                        <div className="div-texte">
                            <h3 className="titre">Farming bread</h3>
                            <p className="texte">Sample text. Click to select the Text Element.</p>
                            <div className="prix">$5.25</div>
                        </div>
                    </div>

                    <div className="div-img-texte">
                        <img src="https://images01.nicepagecdn.com/c461c07a441a5d220e8feb1a/fa992363623553128956b054/hh.jpg" />
                        <div className="div-texte">
                            <h3 className="titre"> Mixed Grain</h3>
                            <p className="texte">Sample text. Click to select the Text Element.</p>
                            <div className="prix">$4.5</div>
                        </div>
                    </div>

                    <div className="div-img-texte">
                        <img src="https://images01.nicepagecdn.com/c461c07a441a5d220e8feb1a/1790791acc975a71ba003ca1/hg.jpg" />
                        <div className="div-texte">
                            <h3 className="titre">Milk Bread</h3>
                            <p className="texte">Sample text. Click to select the Text Element.</p>
                            <div className="prix">$3.55</div>
                        </div>
                    </div>

                    <div className="div-img-texte">
                        <img src="https://images01.nicepagecdn.com/c461c07a441a5d220e8feb1a/c05083dfe0235ffaa9a5cfbf/jhjhjh.jpg" />
                        <div className="div-texte">
                            <h3 className="titre">Brown Bread</h3>
                            <p className="texte">Sample text. Click to select the Text Element.</p>
                            <div className="prix">$2.45</div>
                        </div>
                    </div>

                    <div className="div-img-texte">
                        <img src="https://images01.nicepagecdn.com/c461c07a441a5d220e8feb1a/c1a914846ece539fab7e5b32/j.jpg" />
                        <div className="div-texte">
                            <h3 className="titre"> Cheese Bread</h3>
                            <p className="texte">Sample text. Click to select the Text Element.</p>
                            <div className="prix">$5.5</div>
                        </div>
                    </div>


                </div>
            </div>

            <div className='div2'>
                <h1> Brownies, Pastries, etc</h1>
                <div className='div2-contanier-img-texte'>
                    <div className="div2-img-texte">
                        <img src="https://assets.nicepagecdn.com/d2cc3eaa/3284316/images/fggggg.jpg" />
                        <div className="div-texte">
                            <h3 className="titre"> croissant</h3>
                            <p className="texte">Sample text. Click to select the text box. Click again or double click to start editing the text.</p>
                            <p>More</p>
                        </div>
                    </div>
                    <div className="div2-img-texte">
                        <img src="https://assets.nicepagecdn.com/d2cc3eaa/3284316/images/yyyy.jpg" />
                        <div className="div-texte">
                            <h3 className="titre">  Brownies</h3>
                            <p className="texte">Sample text. Click to select the text box. Click again or double click to start editing the text.</p>
                            <p>More</p>
                        </div>
                    </div>

                    <div className="div2-img-texte">
                        <img src="https://assets.nicepagecdn.com/d2cc3eaa/3284316/images/f5f4b03a-6b27-3595-ba4e-7b894cc961a3.jpg" />
                        <div className="div-texte">
                            <h3 className="titre">  Brownies</h3>
                            <p className="texte">Sample text. Click to select the text box. Click again or double click to start editing the text.</p>
                            <p>More</p>
                        </div>
                    </div>

                    <div className="div2-img-texte">
                        <img src="https://assets.nicepagecdn.com/d2cc3eaa/3284316/images/444.jpg" />
                        <div className="div-texte">
                            <h3 className="titre">  Brownies</h3>
                            <p className="texte">Sample text. Click to select the text box. Click again or double click to start editing the text.</p>
                            <p>More</p>
                        </div>
                    </div>

                    <div className="div2-img-texte">
                        <img src="https://assets.nicepagecdn.com/d2cc3eaa/3284316/images/c91fa21f-6a02-001e-d59f-2125c9c711bd.jpg" />
                        <div className="div-texte">
                            <h3 className="titre">  Brownies</h3>
                            <p className="texte">Sample text. Click to select the text box. Click again or double click to start editing the text.</p>
                            <p>More</p>
                        </div>
                    </div>

                    <div className="div2-img-texte">
                        <img src="https://assets.nicepagecdn.com/d2cc3eaa/3284316/images/96a8f2c7-47a4-66cb-fec2-68902bf475cf.jpg" />
                        <div className="div-texte">
                            <h3 className="titre">  Brownies</h3>
                            <p className="texte">Sample text. Click to select the text box. Click again or double click to start editing the text.</p>
                            <p>More</p>
                        </div>
                    </div>

                    <div className="div2-img-texte">
                        <img src="https://assets.nicepagecdn.com/d2cc3eaa/3284316/images/3ec2a0c1-031e-107f-1950-2cfd3f6314ce.jpg" />
                        <div className="div-texte">
                            <h3 className="titre">  Brownies</h3>
                            <p className="texte">Sample text. Click to select the text box. Click again or double click to start editing the text.</p>
                            <p>More</p>
                        </div>
                    </div>

                    <div className="div2-img-texte">
                        <img src="https://assets.nicepagecdn.com/d2cc3eaa/3284316/images/444666.jpg" />
                        <div className="div-texte">
                            <h3 className="titre">  Brownies</h3>
                            <p className="texte">Sample text. Click to select the text box. Click again or double click to start editing the text.</p>
                            <p>More</p>
                        </div>
                    </div>

                </div>
            </div>

            <div className='div3'>
                <h1>Sweets </h1>
                <div className='div3-flex'>
                    <div>

                        <h3>Cookies</h3>
                        <p> — Biscotti</p>
                        <p> — Chocolate Chip</p>
                        <p> — Oatmeal Pecan</p>
                        <p> — Oat Ginger Bar</p>
                        <p> — Seasonal specials</p>
                    </div>

                    <div>
                        <h3>Scones</h3>
                        <p> — Classic butter scone</p>
                        <p> — Fresh fruit buttermilk scones</p>
                        <p> — Handmade scones</p>
                    </div>

                    <div>
                        <h3>Other</h3>
                        <p> — Seasonal galletes</p>
                        <p> — Tartlets</p>
                        <p> — Banana Bread</p>
                        <p> — Bread Pudding</p>
                        <p> — Cranberry Pecan Granola</p>

                    </div>
                </div>
            </div>

            <div className='div4'>
                <div className="contenu">
                    <img src="https://assets.nicepagecdn.com/d2cc3eaa/3256687/images/65.jpg" />
                    <div className='texte'>
                        <h3> We’d Like to Hear from You</h3>
                        <p>
                            We are leaders in the manufacture and distribution of frozen doughs, with an extensive range of bread and pastries and a management oriented to satisfy the needs of our customers.
                        </p>
                        <p>Image from Freepik</p>
                        <p><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-telephone-fill" viewBox="0 0 16 16">
                            <path fill-rule="evenodd" d="M1.885.511a1.745 1.745 0 0 1 2.61.163L6.29 2.98c.329.423.445.974.315 1.494l-.547 2.19a.678.678 0 0 0 .178.643l2.457 2.457a.678.678 0 0 0 .644.178l2.189-.547a1.745 1.745 0 0 1 1.494.315l2.306 1.794c.829.645.905 1.87.163 2.611l-1.034 1.034c-.74.74-1.846 1.065-2.877.702a18.634 18.634 0 0 1-7.01-4.42 18.634 18.634 0 0 1-4.42-7.009c-.362-1.03-.037-2.137.703-2.877L1.885.511z" />
                        </svg> +1 25 36 89 45 55 </p>
                    </div>
                </div>
            </div>

            <div className='div5'>
                <div className='div5-contenu-recettes'>
                    <div className='div-une-recette'>
                        <img src="https://images01.nicepagecdn.com/c461c07a441a5d220e8feb1a/8baae157ffbe56548790d776/yty.jpg" />
                        <h2>Breads</h2>
                        <p>Sample text. Click to select the text box. Click again or double click to start editing the text.</p>
                    </div>

                    <div className='div-une-recette'>
                        <img src="https://images01.nicepagecdn.com/c461c07a441a5d220e8feb1a/8d481fe712f85027870c837e/rtrtrt.jpg" />
                        <h2>Сookies</h2>
                        <p>Sample text. Click to select the text box. Click again or double click to start editing the text.</p>
                    </div>

                    <div className='div-une-recette'>
                        <img src="https://images01.nicepagecdn.com/c461c07a441a5d220e8feb1a/9765013a9a185d16adf414dd/b1182e8f-cee4-f571-00ff-b553b7f96898.jpg" />
                        <h2>Pastries</h2>
                        <p>Sample text. Click to select the text box. Click again or double click to start editing the text.</p>
                    </div>

                </div>


                <div className='div-Sabonner-Newsletter-Location-Hours'>
                    <div className='div-Sabonner-Newsletter'>
                        <h2> Sign up for the newsletter</h2>

                        <p>Want to be the first to read our news? Subscribe to the newsletter to keep abreast of all events.
                        </p>
                        <input placeholder='Enter a valid email adresse' /> <button>Submit</button>
                    </div>
                    <div className='div-Location-Hours'>
                        <h2> Hours & Location</h2>
                        <div className='adresse'>
                            9 rue du Maréchal, <br />
                            Paris 92 500,  <br />
                            France
                        </div>

                        <p> Monday - Sunday <br /> 7:00am - 8:00pm</p>

                        <p style={{ "font-size": "15.5px" }}>  Image from Freepik</p>
                    </div>

                </div>
            </div>

           

            <div className='div6'>
            <div>
                    <h1>Headline</h1>
                    <p>Sample text. Click to select the Text Element.</p>
                </div>
                <div>
                    <h1>Headline</h1>
                    <p>Sample text. Click to select the Text Element.</p>
                </div>
              
                <div>
                    <h1>Headline</h1>
                    <p>Sample text. Click to select the Text Element.</p>
                    <a href="https://nicepage.com/templates/preview/delicious-breads-4883213?device=desktop" >
                Lien </a>
                </div>
            </div>


          
        </div>
    );
}

export default TemplateWebSite9;